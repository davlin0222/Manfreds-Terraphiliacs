...
document.querySelectorAll('main img').forEach(function(elmnt){
  elmnt.addEventListener('focus',() => {transform(elmnt,0.95)});
  elmnt.addEventListener('blur',() => {transform(elmnt)});
});

document.addEventListener('scroll',function(){
  document.querySelectorAll('main img').forEach(function(elmnt){elmnt.blur();});
});
...
function transform(element,size=0){
  ...
  size = Math.min(window.innerWidth * size / (element.clientWidth**1.9) * 230,window.innerHeight * 0.95 / element.clientHeight);
